...
def findAverageKnownGestureDistance(knownGestures):
    TrainingDistAvg=[]
    numerator=0
    for gesture in knownGestures:
        numerator=numerator+knownGestures[gesture]
    denominator=len(knownGestures)
    TrainingDistAvg=numerator/denominator
    print(TrainingDistAvg)
    return TrainingDistAvg
    ...
    if train==True:
        if handsLM!=[]: #handslm will be empty if there is no hand in the frame.
            ...
            if cv2.waitKey(1) & 0xff == ord('t'):
                ...
                trainCnt=trainCnt+1
                if trainCnt==numGest:
                    avgTrainedDistances=findAverageKnownGestureDistance(knownGestures)

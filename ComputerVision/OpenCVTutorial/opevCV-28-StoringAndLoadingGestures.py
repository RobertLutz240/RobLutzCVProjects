...
def findDistances(handData):
    ...
    palmSize=((handData[0][0]-handData[9][0])**2+(handData[0][1]-handData[9][1])**2)**(1./2.) #calculates the distance between palm and base of middlefinger
...
if train==1: #if we say we want to train, then:
    trainCnt=0 #set train count to zero
    knownGestures=[] #make an empty array to hold gestures
    numGest=int(input("how many gestures do you want? ")) #number of gestures
    gestNames=[] #make an array for holding names of gestures
    ...
    print("gestNames= ",gestNames)
    trainName=input('Filename for training data? (Press Enter for Default) ')
    if trainName=='': #if use hit enter without typing anything
        trainName='default'
...
tol=10 #arbitrary tolerance value
    ...
    if train==1:
        if handsLM!=[]: #handslm will be empty if there is no hand in the frame.
            ...
            if cv2.waitKey(1) & 0xff == ord('t'):
                ...
                trainCnt=trainCnt+1
                if trainCnt==numGest:
                    train=0 #set training to false once youve trained the number of gestures youve typed in
                    with open(trainName,'wb') as f: #wb = write byte. rb = read byte
